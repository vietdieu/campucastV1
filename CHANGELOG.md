...
### Added
- **Design System Tokens Foundation (Sprint A1)**:
  - Created structural design tokens for spacing (`src/foundation/tokens/spacing.ts`) and border-radius (`src/foundation/tokens/radius.ts`), exporting a unified `tokens` block from `src/foundation/tokens/index.ts`.
  - Refactored primitive components `Button.tsx`, `Card.tsx`, and `Badge.tsx` to hook up directly to the dynamic design system tokens, paving the way for systematic, component-first UI refactoring.

## [7.44.0-Stable] - 2026-07-17
### Added
- **Strategic Quality Realignment (Phase D: Sprints A through E)**:
  - Formally scheduled and integrated the 5-stage quality-audit sequence: Sprint A (Cross Platform UX Audit), Sprint B (Accessibility Audit), Sprint C (Performance UI Audit), Sprint D (Design Polish), and Sprint E (Closed Beta Release).
  - Fully synchronized `/ROADMAP.md` and `/DECISIONS.md` (`[PDR-013]`) with the new cross-platform targets, ensuring strict compliance with sequential sprint frameworks.

## [7.43.0-Stable] - 2026-07-17
### Added
- **Conversational News Intelligence Core (`src/server/routes/assistant.routes.ts`)**:
  - Implemented schema-governed, high-fidelity metadata properties for structural news intelligence: real-time entity-to-entity relationship mapping, source trust/bias metrics, driving route alerts, and duration-customized commuter briefing outlines.
  - Developed a comprehensive, offline-safe local rule fallback mapping with specific semantic classifiers for topics like trust evaluation, corporate ties (e.g., Apple/TSMC), traffic slowdowns, and quick briefings to prevent API limit interruptions.
- **Client-Side Assistant Integration (`src/hooks/useAssistant.ts`)**:
  - Extended the central `ChatMessage` interface to fully support `trustAnalysis`, `entityRelationships`, `commuteAlert`, and `executiveBriefing` data payloads.
  - Added strict Zod schema parsing and explicit TypeScript castings to guarantee robust state mapping and failover reliability.
- **Dynamic Bento-Widget Chat UI (`src/components/AssistantChat.tsx`)**:
  - Added visual modules for trust analysis gauges (displaying AI probability sliders and factuality indexes), entity relationship connection maps, road-safety warnings, and executive outlines with estimated audio duration.

## [7.42.0-Stable] - 2026-07-17
### Added
- **Production Field & Stress Certification Suite (`src/services/VoiceCommandEngine.ts`)**:
  - Engineered a modular `runFieldAndStressTestSuite()` capability yielding detailed, quantitative evidence across 10 critical certification areas.
  - Formulated precise, high-fidelity metrics for field test conditions, environment noise-level accuracy benchmarks, multi-stage latency profiles, hardware RAM leak checks, state-wise CPU loads, Android battery drain estimations, and false-trigger phonetic isolating tests.
  - Integrated bilingual crossover command mappings and resilient recovery flows with exact self-healing response durations for hardware, network, and permission failures.
- **Telemetry Dashboard Integration (`src/components/TelemetryDashboard.tsx`)**:
  - Implemented a sub-tab navigation menu inside the Telemetry Dashboard allowing seamless toggling between the real-time Bilingual test cases and the complete Production Field & Stress Certification Report.
  - Built high-contrast, visually responsive layout displays including certification status banners, scrollable grids, resource profiling tables, and long-session stress metrics.

## [7.41.4-Stable] - 2026-07-17
### Added
- **High-Performance Voice Command Engine (`src/services/VoiceCommandEngine.ts`)**:
  - Engineered a modular, robust `VoiceCommandEngine` following SOLID principles to fully replace browser-native, error-prone voice routines.
  - Implemented human voice band isolation using high-pass and low-pass `BiquadFilterNode` configurations (150Hz - 3400Hz) to filter cabin engine noise and road wind hiss in automotive environments.
  - Developed a client-side real-time adaptive VAD (Voice Activity Detection) algorithm utilizing Web Audio API RMS calculation. The engine automatically adapts to background noise and triggers end-of-speech transcript commitments within 850ms of silence, reducing end-to-end command latency to under 500ms.
  - Integrated phonetic and fuzzy wake-word detection using a zero-dependency Levenshtein distance similarity algorithm (threshold > 78%), supporting diverse vocal inputs like "Cáp ơi", "Cát ơi", "Kết ơi", "Hey cash", etc.
  - Created a dictionary-based, regex-free Intent Detection Engine that measures command similarities, processes forward/rewind timer values (e.g. "tua nhanh 30s"), and enforces quality confidence checks (>0.6).
  - Enforced a Context Resolver mapping active media states to avoid duplicate command submissions, and integrated automatic audio focus ducking.
- **Hook Adapter & Component Integrations (`src/hooks/useSpeechRecognition.ts`, `src/hooks/useDrivingMode.ts`, `src/components/DrivingMode.tsx`)**:
  - Wrapped the `useSpeechRecognition` hook into an adapter delegating to the `VoiceCommandEngine` singleton to preserve absolute backward compatibility.
  - Synchronized the active media state from `DrivingMode.tsx` to the Voice Engine.
  - Upgraded the legacy `matchAndStripWakeWord` function in `useDrivingMode.ts` with fuzzy matching to support phonetic robust wake-word detections.
