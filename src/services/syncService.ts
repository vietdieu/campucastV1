...
export async function clearCloudDataAsync(): Promise<boolean> {
  const supabase = await getSupabaseClientAsync();
  if (!supabase) return false;
  ...
  const userId = session.user.id;
  try {
    console.log(`[Sync] Clearing all cloud data for User: ${userId}`);
    
    // 1. Xóa briefings trên cloud
    const { error: err1 } = await supabase
      .from("briefings")
      .delete()
      .eq("user_id", userId);

    // 2. Xóa voice_history trên cloud
    const { error: err2 } = await supabase
      .from("voice_history")
      .delete()
      .eq("user_id", userId);

    // 3. Xóa user_preferences trên cloud
    ...
    if (err1 || err2 || err3) {
      console.error("[Sync] Error clearing cloud tables:", err1 || err2 || err3);
      return false;
    }
    return true;
  } catch (err) {
    console.error("[Sync] Exception during cloud data purge:", err);
    return false;
  }
}

export function getLastSyncTimestamp(): string | null {
  return localStorage.getItem("commutecast_last_sync_at");
}
