...
export function normalizeBriefing(item: any): SavedSummary {
  ...
  const title = item.title || payload.title || "CommuteCast Briefing";
...
export async function saveVoiceHistory(item: Partial<VoiceHistoryItem>): Promise<VoiceHistoryItem> {
  ...
  if (!isIndexedDBSupported()) {
    try {
      ...
      console.warn("Failed to save voice intelligence to localStorage", err);
    }
    return record;
  }
  ...
  try {
    ...
    console.warn("Failed to save voice intelligence to IndexedDB", err);
  }
  return record;
}
...
export async function getVoiceHistory(): Promise<VoiceHistoryItem[]> {
  ...
  try {
    ...
    console.warn("Failed to get voice intelligence", err);
    return [];
  }
}
...
export async function clearVoiceHistory(): Promise<void> {
  ...
  try {
    ...
    console.warn("Failed to clear voice intelligence", err);
  }
}
...
export async function clearAllLocalStores(): Promise<void> {
  ...
  try {
    ...
    console.warn("Failed to clear local IndexedDB stores", err);
    throw err;
  }
}