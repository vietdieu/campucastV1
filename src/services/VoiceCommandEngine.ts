...
export class VoiceCommandEngine {
  ...
  public registerCallbacks(contextId: string, handlers: {
    ...
    if (handlers.onStateChange) {
      this.stateChangeCallbacks.set(contextId, handlers.onStateChange);
  ...
  private updateState(newState: VoiceEngineState) {
    this.state = newState;
    if (this.stateChangeCallbacks.size > 0) {
  ...
  public async startEngine(options: { 
    continuous?: boolean, 
    contextId?: string, 
    wakeWordEnabled?: boolean,
    subscriberId?: string 
  } = {}): Promise<void> {
    const subId = options.subscriberId || options.contextId || 'default';
    ...
    if (options.wakeWordEnabled !== undefined) {
      this.wakeWordEnabled = options.wakeWordEnabled;
    }

    if (this.isListeningActive) {
      if (options.contextId && this.activeContextId !== options.contextId) {
        console.log(`[VoiceCommandEngine] Context switching from ${this.activeContextId} to ${options.contextId}`);
        this.activeContextId = options.contextId;
        // Verify stream health on context switch
        if (this.mediaStream) {
            const activeTracks = this.mediaStream.getTracks().filter(t => t.enabled && t.readyState === 'live');
            console.log(`[VoiceCommandEngine] Context switch - Stream health: ${activeTracks.length}/${this.mediaStream.getTracks().length} tracks active.`);
        }
        // Force recognition restart to refresh callback context and reset session
        if (this.recognition) {
          try {
            this.recognition.stop();
          } catch (e) {
            console.warn("[VoiceCommandEngine] Error stopping recognition during context switch:", e);
          }
        }
      }
      ...
      console.warn("[VoiceCommandEngine] Engine is already listening for subscriber:", subId);
    ...
    this.updateState(this.wakeWordEnabled ? "WAITING_WAKE_WORD" : "LISTENING");
    ...
    try {
      ...
      // High-pass filter to isolate vehicle engine low frequency rumbles (< 100Hz)
      this.hpFilter = audioCtx.createBiquadFilter();
      this.hpFilter.type = "highpass";
      this.hpFilter.frequency.setValueAtTime(100, audioCtx.currentTime);
      this.dspHighPassConnected = true;

      // Low-pass filter to block high frequency hiss / wind whistling (> 8000Hz)
      // Increased to 8kHz to preserve higher frequency speech components for better AI recognition
      this.lpFilter = audioCtx.createBiquadFilter();
      this.lpFilter.type = "lowpass";
      this.lpFilter.frequency.setValueAtTime(8000, audioCtx.currentTime);
      ...
      if (this.errorCallbacks.size > 0) {
        this.errorCallbacks.forEach(cb => cb("error", msg, err.message));
      }
      this.stopEngine();
    }
  }
  ...
  private startVADLoop() {
    ...
    const silenceTimeoutMs = 1200;   // Increased to 1.2s for more natural pauses (avoiding cutting off mid-thought)
  ...
  private triggerDucking(isDucked: boolean) {
    if (this.duckingChangeCallbacks.size > 0) {
      // 0.20 for clear volume ducking, 1.0 for normal.
      const targetVolume = isDucked ? 0.20 : 1.0;
      this.duckingChangeCallbacks.forEach(cb => cb(isDucked, targetVolume));
    }
  }
  ...
  private initWebSpeechAPI() {
    ...
    if (!SpeechRecognitionClass) {
      ...
      if (this.errorCallbacks.size > 0) {
        this.errorCallbacks.forEach(cb => cb("unsupported", msg, "api-missing"));
      }
      return;
    }
    ...
    recognition.onerror = (event: any) => {
      ...
      if (this.errorCallbacks.size > 0) {
        this.errorCallbacks.forEach(cb => cb(type, msg, event.error));
      }
    };
  ...
  public processTranscript(rawText: string) {
    ...
    // Deduplication check: ignore identical transcript within 800ms to prevent "repeating replies"
    const now = Date.now();
    if (rawText.trim() === this.lastProcessedTranscript && (now - this.lastProcessedTime) < 800) {
      console.log("[VoiceCommandEngine] Deduplicating repeated transcript:", rawText);
      return;
    }
    this.lastProcessedTranscript = rawText.trim();
    this.lastProcessedTime = now;

    const startTime = Date.now();
    if (this.transcriptCallbacks.size > 0) {
      if (this.activeContextId && this.transcriptCallbacks.has(this.activeContextId)) {
        this.transcriptCallbacks.get(this.activeContextId)!(rawText);
      } else {
        // Fallback: broadcast to all if no active context is set to ensure reactivity
        this.transcriptCallbacks.forEach(cb => cb(rawText));
      }
    }
    ...
    if (this.wakeWordEnabled) {
      ...
      if (!wakeWordMatch.matched) {
        ...
        if (this.commandCallbacks.size > 0) {
          this.commandCallbacks.forEach(cb => cb({
            type: "UNRECOGNIZED",
            raw: rawText,
            confidence: 0,
          }));
  ...
  private async processCommandQueue() {
    ...
    while (this.commandQueue.length > 0) {
      ...
      if (this.commandCallbacks.size > 0) {
        const cb = this.activeContextId ? this.commandCallbacks.get(this.activeContextId) : null; if (cb) cb(action);
      }
  ...
  // Stop / Shutdown
  public stopEngine(subscriberId?: string) {
    if (subscriberId) {
    ...
    if (this.subscribers.size > 0) {
      console.log(`[VoiceCommandEngine] Stop requested, but ${this.subscribers.size} subscribers still active. Staying alive.`);
      return;
    }
    ...
    if (this.mediaStream) {
      this.mediaStream.getTracks().forEach(track => {
        track.stop();
        console.log(`[VoiceCommandEngine] Audio track stopped: ${track.label}`);
      });
