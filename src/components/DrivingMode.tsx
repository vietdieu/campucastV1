...
export default function DrivingMode({
  ...
  useEffect(() => {
    ...
    engine.setPlayerPlayingState(isPlaying);
  }, [isPlaying]);
  ...
  return (
    ...
    >
      ...
      <header className="shrink-0 h-10 md:h-12 flex items-center justify-between px-2 md:px-4 border-b border-white/5 z-30">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center"
               style={{ backgroundColor: `${colors.interactive}33`, borderColor: `${colors.interactive}4d`, borderWidth: '1px' }}>
      ...
      <main className="w-full flex flex-row items-stretch gap-3 md:gap-4 px-4 flex-1 overflow-hidden py-2">
        ...
        <div className="flex-[4] h-full flex flex-col items-center justify-center bg-[var(--color-surface-overlay)]/50 rounded-2xl overflow-hidden relative p-6">
        ...
        <div className="relative flex items-center justify-between px-4 md:px-8 h-16 border-t border-white/5 pt-2">
          ...
          <div className="flex flex-col gap-2 z-10">
            <div className="flex items-center gap-6">
              {isOffline ? (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[var(--color-warning)]/10 border border-[var(--color-warning)]/20 text-[var(--color-warning)]/80">
                  <WifiOff className="w-3 h-3" />
                  <span className="text-[10px] font-bold uppercase tracking-wider">{t.voiceOffline}</span>
                </div>
              ) : (
                <button onClick={() => setIsContinuous(!isContinuous)} className={cn("flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-semibold tracking-normal transition-all", isContinuous ? "bg-[var(--color-success)]/20 text-[var(--color-success)] border border-[var(--color-success)]/30" : "bg-white/5 text-white/30 border border-white/10")}>
          ...
          <div className={cn(
            ...
            <button 
              ...
              aria-label={uiLanguage === "vi" ? "Tua lại 15 giây" : "Rewind 15 seconds"}
              className="rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-all active:scale-90 w-20 h-20 md:w-24 md:h-24"
            >
              <RotateCcw className="text-white/60 w-8 h-8 md:w-10 md:h-10" />
            </button>

            <button 
              onClick={handleActivePlayPause} 
              aria-label={activeIsPlaying ? (uiLanguage === "vi" ? "Tạm dừng" : "Pause") : (uiLanguage === "vi" ? "Phát bản tin" : "Play")}
              className="rounded-full bg-white text-black shadow-xl flex items-center justify-center hover:bg-zinc-200 transition-all active:scale-95 w-24 h-24 md:w-28 md:h-28"
            >
              {activeIsPlaying ? (
                <Pause className="fill-current w-10 h-10 md:w-12 md:h-12" />
              ) : (
                <Play className="fill-current ml-1.5 w-10 h-10 md:w-12 md:h-12" />
              )}
            </button>

            <button 
              onClick={() => handleActiveSkip(15)} 
              aria-label={uiLanguage === "vi" ? "Tua đi 15 giây" : "Fast forward 15 seconds"}
              className="rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-all active:scale-90 w-20 h-20 md:w-24 md:h-24"
            >
              <RotateCw className="text-white/60 w-8 h-8 md:w-10 md:h-10" />
            </button>

            {onNext && (
              <button 
                onClick={handleActiveNext} 
                aria-label={uiLanguage === "vi" ? "Chuyển bản tin tiếp theo" : "Next briefing"}
                className="rounded-full border border-white/10 bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all active:scale-90 w-20 h-20 md:w-24 md:h-24"
              >
                <SkipForward className="text-blue-400 w-8 h-8 md:w-10 md:h-10" />
              </button>
            )}
          </div>
