...
export default function SampleBriefings({ saveNewBriefing, onPlaySample, uiLanguage = "vi" }: SampleBriefingsProps) {
  ...
  const handlePlaySampleClick = async (sample: typeof samples[0]) => {
    ...
    try {
      ...
      const savedItem: SavedSummary = {
        id: `sample-${Date.now()}`,
        title: sample.payload.title || "Sample Briefing",
  ...
  return (
    <div className="rounded-3xl border p-5 flex flex-col gap-4 text-left" id="sample-briefings-panel"
      ...
      <div>
        <h4 className="text-sm font-bold uppercase tracking-wider flex items-center gap-1.5"
          ...
          <Sparkles className="w-4.5 h-4.5 animate-pulse" style={{ color: colors.warning }} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
        {samples.map((sample) => {
          ...
          return (
            <div
              key={sample.id}
              className="border p-4 rounded-2xl flex flex-col justify-between gap-3 shadow-2xs transition"
              style={{ backgroundColor: colors.surface, borderColor: colors.border }}
            >
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border"
                    style={sample.language === "vi" 
                      ? { backgroundColor: `${colors.critical}1a`, color: colors.critical, borderColor: `${colors.critical}33` } 
                      : { backgroundColor: `${colors.interactive}1a`, color: colors.interactive, borderColor: `${colors.interactive}33` }
                    }>
                    {sample.tag}
                  </span>
                  <span className="text-[10px] font-mono" style={{ color: colors.textMuted }}>27s Synth Feed</span>
                </div>
                <h5 className="text-xs font-extrabold tracking-tight leading-snug" style={{ color: colors.textPrimary }}>
                  {sample.title}
                </h5>
                <p className="text-[11px] mt-1 line-clamp-2" style={{ color: colors.textSecondary }}>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-xs font-bold py-2 px-3 rounded-xl transition cursor-pointer active:scale-97 disabled:opacity-50 border"
                style={isLoading
                  ? { backgroundColor: colors.surfaceOverlay, color: colors.interactive, borderColor: colors.border }
                  : { backgroundColor: colors.interactive, color: colors.onAccent, borderColor: "transparent" }}
              >
                {isLoading ? (
                  <>
                    <span className="w-3.5 h-3.5 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: colors.interactive, borderTopColor: "transparent" }}></span>
