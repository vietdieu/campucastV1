    ...
    caches.match(event.request)
      .then((response) => {
        ...
        return response || fetch(event.request).then((networkResponse) => {
          // Don't cache range requests or failed responses (status 0 is for opaque cross-origin resources, which is acceptable for cached fonts/assets)
          if (!networkResponse || (networkResponse.status !== 200 && networkResponse.status !== 0)) {
            return networkResponse;
          }

          // Dynamic caching of assets (css, js, assets, fonts, CDNs etc)
